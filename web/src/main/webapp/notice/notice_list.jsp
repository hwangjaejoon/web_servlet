...
<%@ page language="java" contentType="text/html; charset=UTF-8"
 ...
 /*
  페이지 번호 생성
  페이징 생성 방법
  1. 한 페이지 당 몇 개씩 데이터를 출력할 것인지 설정
  2. 데이터베이스에 있는 데이터의 총 갯수 / 한 페이지당 갯수(소숫점으로 나옴)
  3. Math.ceil 사용하는 이유는 소숫점으로 나왔을 때 올림으로 페이지를 추가되도록 함
 */
 ...
 //최초 공지사항 리스트 페이지에 접근 시 페이지 번호가 없을 수 있음 또는 페이지 번호 1을 클릭 시
...
<body>
<p>현재 등록 된 게시물 : <%=notice.get(0).get(5) %></p>
...
<%	int f;
	...
	//총 데이터 갯수 - ((페이지번호 -1) * 출력할 갯수 )
	int total = Integer.parseInt(total_page) - ((Integer.parseInt(pno) - 1) * 3);		//리스트 출력 번호를 출력하기 위한 변수
